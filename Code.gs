...
// 查某姓名是否已綁定 LINE(回傳本人 userId;排除來賓 G:、代報名 M: 佔位)
function boundUserIdByName_(name) {
  var d = membersData_();
  for (var i = 1; i < d.length; i++) {
    if (String(d[i][1] || '') === name) {
      var uid = String(d[i][0] || '');
      if (uid && uid.indexOf('G:') !== 0 && uid.indexOf('M:') !== 0) return uid;
    }
  }
  return '';
}
function proxyRegister(userId, p) {
  ...
  try {
    var sh = sheet(SHEET_ROSTER), data = sh.getDataRange().getValues();
    for (var i = 1; i < data.length; i++) {
      if (String(data[i][1] || '') === name) return { ok: false, error: '「' + name + '」已在出席名單' };   // 一人一筆,防重複
    }
    var hmap = getHcpMap();
    var hcp = hmap.hasOwnProperty(name) ? String(hmap[name]) : '';
    var cart = (p.cart || '用餐').toString().trim();
    var bound = boundUserIdByName_(name);                 // 已綁定 → 用本人 userId(他自己 App 看得到、可改);未綁定 → M: 佔位
    var uid = bound || ('M:' + name);
    sh.appendRow([uid, name, hcp, cart, '代報名', Date.now(), '報名']);
  } finally { lock.releaseLock(); }
  __rosterCache = null; cacheDel_('roster');
...
function proxyRemove(userId, p) {
  ...
  try {
    ...
    for (var i = 1; i < data.length; i++) {
      // 只移除「代報名」註記的那一筆(不會誤刪本人自行報名的列)
      if (String(data[i][1] || '') === name && String(data[i][4] || '') === '代報名') { row = i + 1; break; }
    }
...
// 「本場之後」的下一場(PDF 用:報告 7/21 → 帶 8/18,不受今天日期影響)
function nextEventAfterDate_(cfg, dateStr) {
  var sched = getSchedule(), curKey = 0;
  for (var i = 0; i < sched.length; i++) { if (mdOf_(sched[i].date) === mdOf_(dateStr)) { curKey = sched[i].sortKey; break; } }
  if (!curKey) curKey = annDateKey_(dateStr);
  for (var j = 0; j < sched.length; j++) { if (sched[j].sortKey > curKey) return sched[j]; }
  return null;
}
...
function buildMatchReportHtml(date, cfg) {
  ...
  // 下次例賽(本場之後的下一場)
  var nx = nextEventAfterDate_(cfg, date);
  ...
  if (nx) ni.push('下場例賽:' + nx.date + (nx.dow ? '(' + String(nx.dow).replace('週', '') + ')' : '') + ' · ' + nx.course + (nx.tee ? ' · ' + nx.tee : ''));
  if (ni.length) {
...
function annDateKey_(md) {
  ...
  if (cand - now > 183 * 86400000) y -= 1;               // 超過半年後的日期 → 視為去年
  return y * 10000 + mo * 100 + d;
}
function getAnnouncements() {
  var sh = sheet(SHEET_ANNOUNCE), data = sh.getDataRange().getValues();
  // 一次性搬移舊的單一 announce 設定 → 公告分頁
  if (data.length < 2) {
    var cfg0 = getConfig();
    if (cfg0.announce && String(cfg0.announce).trim()) {
      var t = new Date();
      sh.appendRow([(t.getMonth() + 1) + '/' + t.getDate(), String(cfg0.announce).trim(), Date.now()]);
      setConfig('announce', '');
...
function addAnnouncement(userId, p) {
  var cfg = getConfig();
  if (!userId || userId !== cfg.adminUserId) return { ok: false, error: '只有管理員可以發布公告' };
  var text = String(p.text || '').trim();
  if (!text) return { ok: false, error: '請填公告內容' };
  var date = String(p.date || '').trim();
  if (!date) { var t = new Date(); date = (t.getMonth() + 1) + '/' + t.getDate(); }
  sheet(SHEET_ANNOUNCE).appendRow([date, text, Date.now()]);
  return { ok: true, announcements: getAnnouncements() };
}
function removeAnnouncement(userId, p) {
  var cfg = getConfig();
  if (!userId || userId !== cfg.adminUserId) return { ok: false, error: '只有管理員可以刪除公告' };
  var id = String(p.id || '');
  var sh = sheet(SHEET_ANNOUNCE), data = sh.getDataRange().getValues();
  for (var i = data.length - 1; i >= 1; i--) {
